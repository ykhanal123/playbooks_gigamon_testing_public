...
# Run example:
# ./check_int.py eno6 eno7
# List interfaces after filename with spaces
# it then writes out the result to a file initial.txt with bytes first then packets then drops.
# this will be analyzed in the check_int_dif script
...
def interface_diff(count):
        ...
        for interface in interfaces:
            ...
            # write files to use for analysis
            if numInt==1:
                operation="w"
            else:
                operation="a"
            with open("initial.txt", operation) as initialOutput:
                initialOutput.write("%s %s %s\n" % (formatted[0], formatted[1], formatted[3]))
            numInt+=1

        # write out info to stdout
        print("")
        print("Run: " + str(count))
